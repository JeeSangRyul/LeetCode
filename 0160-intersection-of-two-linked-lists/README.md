...
둘이 총 이동거리 **A+B** 를 동일하게 돈다.

그래서 공통되는 tail 구간에서 같은 시점에 만난다.

즉,

> 각 포인터는 공통 tail에 도달하기 전에
서로 다른 길이를 자동으로 보정한다.
> 
...
- 처음엔 A, B 다른 길이를 걷는다
- 끝나면 서로 반대 리스트로 간다
- 결국 둘 다 **A+B 동일한 총 길이**를 걷는다
- 그래서 공통 구간 시작점에서 만난다
- 못 만나면 둘 다 nil 도착 → nil 반환
...
전체 거리 같아지고 결국 어느 지점에서든 동시에 만나게 됨.
